...
import static com.example.jkmusicplayer.MainActivity.*;
...
public class PlayMusicActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        HashMap<Object, Object> songsLists = null;
        ...
        Log.d("1234567", "onCreate: "+ songsLists.get(position).getSongPath());


        bitmap = songsLists.get(position).getBitmap();


            try {
                mp.setDataSource(this, Uri.parse(songsLists.get(position).getSongPath()));
                mp.prepareAsync();
        ...
        tvDuration.setText(String.format("%s:%s",
                songsLists.get(position).getMinlength(),
                songsLists.get(position).getSeclength()));
    ...
    public void seekUpdation()
    {
        ...
        tvCurrentTime.setText(String.format("%s:%s", String.valueOf(minut), String.valueOf(se)));
    ...
    public void StartMediaPlayer(int position)
    {
        try {
            mp.reset();
            mp.setDataSource(getApplicationContext(),Uri.parse(songsLists.get(position).getSongPath()));
            mp.prepareAsync();
            final SongsList thisSong = songsLists.get(position);
            mp.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                @Override
                public void onPrepared(MediaPlayer mp) {
                    mp.start();
                    ...
                    tvDuration.setText(String.format("%s:%s", thisSong.getMinlength().toString(), thisSong.getSeclength().toString()));
